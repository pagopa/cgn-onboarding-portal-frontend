import React from "react";
import InfoCircleIcon from "../../assets/icons/info-circle.svg";
import ActivitiesItem from "./ActivitiesItem";

const Activities = () => (
  <section className="bg-white">
    <header className="pr-6 pt-6 pl-6">
      <div className="d-flex justify-content-between">
        <h1 className="text-base text-uppercase font-weight-semibold tracking text-dark-blue">
          Attività
        </h1>
        <InfoCircleIcon />
      </div>
      <p className="text-sm mb-0">
        Questa è la lista della attività richieste agli operatori per aderire
        all’iniziativa
      </p>
    </header>
    <div className="d-flex flex-column">
      <ActivitiesItem
        description="Inserisci i dati di contatto e la descrizione dei beni e servizi offerti"
        checked
        hasBorderBottom
      >
        <h1 className="text-base font-weight-semibold text-dark-blue">
          Compila i dati Operatore
        </h1>
      </ActivitiesItem>
      <ActivitiesItem
        description="Descrivi almeno una delle offerte destinate ai titolari di CGN"
        checked={false}
        hasBorderBottom
      >
        <h1 className="text-base font-weight-semibold text-dark-blue">
          Inserisci un’agevolazione
        </h1>
      </ActivitiesItem>
      <ActivitiesItem
        description="Scarica, compila e firma i documenti. Poi caricali sul portale"
        checked={false}
        hasBorderBottom={false}
      >
        <h1 className="text-base font-weight-semibold text-dark-blue">
          Compila, firma e carica
          <br /> i documenti
        </h1>
      </ActivitiesItem>
    </div>
  </section>
);

export default Activities;
